...
colnames(biking)[1:2] <- c("start_time", "end_time")
biking
```
...
```{r}
trip_duration <- as.duration(biking$end_time - biking$start_time)/dminutes(1)
...
```{r}
library(stringr)
telephone <- c("615-936-8320", "615.936.8320")
patterns <- c("[0-9*]+-+[0-9*]+-+[0-9*]", "[0-9*]+.+[0-9*]+.+[0-9*]")
format_tn <- str_detect(telephone, pattern = patterns)
```
